...
export default {
  ...
  name: 'sketchfab',
  props: ['urlid','autospin','autostart','preload','ui_controls','ui_infos','ui_related', 'camera'],
  components: {},
  data () {
    return {
      cam: {
  ...
  mounted(){
  	var self = this
  	var client = new Sketchfab( this.$refs.api_iframe )
  ...
  computed:{
	...Vuex.mapGetters(['_map', 'sketchfab', 'mapbox']),
  ...
  watch:{
    ...
    FOV: function(val){
      ...
      this._api.getCameraLookAt( function( err, camera ){ self.sk = camera } )
      if(!self.sketchfabOrigin){self.skOrigin = [camera.position.map(e=>{return parseInt(e)}), camera.target.map(e=>{return parseInt(e)})]}
